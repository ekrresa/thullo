...
type CardInfo = { id: number; board_id: number; listTitle: string };
...
const CardContext = React.createContext<ICardContext>({
  cardInfo: { id: 0, board_id: 0, listTitle: '' },
  handleCardInfo: () => {},
  openCardModal: false,
  handleCardModal: () => {},
});
...
export function CardProvider({ children }: React.PropsWithChildren<unknown>) {
  const [cardInfo, setCardInfo] = React.useState({ id: 0, board_id: 0, listTitle: '' });
